...
function generateString(cfg) {
	...
	const max =
		cfg.stringMax || cfg.stringLength ? cfg.stringLength - cfg.stringMin : 10;
		...
		random: function (cfg = {}) {
			return function () {
				...
				const length =
					...
					getRandomValue(
						cfg.sentenceMin || 1,
						cfg.sentenceMax || cfg.sentenceLength
							? cfg.sentenceLength - cfg.sentenceMin
							: 10
					);
...
class Generator {
	constructor(config) {
		...
		config.forEach((cfg) => {
			this.addField(new Path(cfg.path), cfg.generator, cfg.options);
		});
	}
